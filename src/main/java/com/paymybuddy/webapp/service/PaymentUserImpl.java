...
public class PaymentUserImpl implements IPayment {
	...
	public void pay(String userEmail,double amount,String typeAccountBeneficairyUser) {
		...
		try {
			this.updateBalanceBuddyAccountsContactAndUserWithFeesTransaction(userEmail, typeAccountBeneficairyUser);
		} catch (Exception e) {
	...
	public void calculBalance(String userEmail, double amount,String typeAccountBeneficiary) {
		...
		if (typeAccountBeneficiary.equals(Constants.BANKING_ACCOUNT)) {
			balanceCalculatedBeneficiaryUser = operation.add(balanceBankingAccount, amount);
			balanceCalculatedCreditUser = operation.withdraw(balanceBuddyAccount, amountWithFeesTransaction);
		} else if (typeAccountBeneficiary.equals(Constants.BUDDY_ACCOUNT)) {
			balanceCalculatedBeneficiaryUser = operation.add(balanceBuddyAccount, amount - feesTransaction);
			...
			balanceCalculatedCreditUser = operation.withdraw(balanceBankingAccount, amount);
		}
	}
